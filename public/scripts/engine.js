function Runner(container, config) {
	...
	(Runner.instance_ = this),
		...
		this.isDisabled()
			...
			: (Runner.isAltGameModeEnabled() && (this.initAltGameType(), (Runner.gameType = this.gameType)),
				this.loadImages(),
...
function getA11yString(key) {
	return loadTimeData && loadTimeData.valueExists(key) ? loadTimeData.getString(key) : '';
}
...
function createCanvas(parentElement, width, height, additionalClass) {
	...
	canvas.className = additionalClass ? Runner.classes.CANVAS + ' ' + additionalClass : Runner.classes.CANVAS;
	canvas.width = width;
...
function GameOverPanel(canvas, textImagePosition, restartImagePosition, canvasDimensions, altGameEndImagePosition, altGameModeActive) {
	this.canvas = canvas;
...
function checkForCollision(tRex, obstacle, canvasContext) {
	const collisionBox1 = new CollisionBox(obstacle.xPos + 1, obstacle.yPos + 1, obstacle.config.WIDTH - 2, obstacle.config.HEIGHT - 2),
		collisionBox2 = new CollisionBox(tRex.xPos + 1, tRex.yPos + 1, tRex.typeConfig.width * tRex.size - 2, tRex.typeConfig.height - 2);
	if ((canvasContext && drawCollisionBoxes(canvasContext, collisionBox1, collisionBox2), boxCompare(collisionBox1, collisionBox2))) {
		...
		for (let i = 0; i < trexBoxes.length; i++) {
			for (let j = 0; j < tRexCollisionBoxes.length; j++) {
				const adjustedBox1 = createAdjustedCollisionBox(trexBoxes[i], collisionBox1),
					adjustedBox2 = createAdjustedCollisionBox(tRexCollisionBoxes[j], collisionBox2),
					isColliding = boxCompare(adjustedBox1, adjustedBox2);
				if ((canvasContext && drawCollisionBoxes(canvasContext, adjustedBox1, adjustedBox2), isColliding)) {
...
function createAdjustedCollisionBox(collisionBox, offsetBox) {
	return new CollisionBox(collisionBox.x + offsetBox.x, collisionBox.y + offsetBox.y, collisionBox.width, collisionBox.height);
}
...
function Obstacle(canvasContext, typeConfig, spritePosition, dimensions, gapCoefficient, speed, xOffset, altGameModeActive) {
...
(Runner.config = {
	...
	(Runner.prototype = {
		initAltGameType() {
			GAME_TYPE.length > 0 &&
				(this.gameType =
					loadTimeData && loadTimeData.valueExists('altGameType') ? GAME_TYPE[parseInt(loadTimeData.getValue('altGameType'), 10) - 1] : '');
		},
		...
		loadImages() {
			...
			(this.spriteDef = Runner.spriteDefinition.LDPI),
				...
				(Runner.altGameImageSprite && Runner.altCommonImageSprite) ||
					((Runner.isAltGameModeEnabled = () => !1), (this.altGameModeActive = !1)),
		...
		init() {
			(document.querySelector('.' + Runner.classes.ICON).style.visibility = 'hidden'), this.adjustDimensions(), this.setSpeed();
		...
		adjustDimensions() {
			...
			(this.dimensions.WIDTH = this.outerContainerEl.offsetWidth - 2 * e),
				this.isArcadeMode() &&
					((this.dimensions.WIDTH = Math.min(600, this.dimensions.WIDTH)), this.activated && this.setArcadeModeContainerScale()),
				this.canvas &&
					...
					this.crashed &&
						this.gameOverPanel &&
						(this.gameOverPanel.updateDimensions(this.dimensions.WIDTH), this.gameOverPanel.draw(this.altGameModeActive, this.tRex)));
		},
		playIntro() {
		...
		update() {
			...
			if (
				(this.altGameModeFlashTimer < 0 || 0 === this.altGameModeFlashTimer
					? ((this.altGameModeFlashTimer = null), this.tRex.setFlashing(!1), this.enableAltGameMode())
					: this.altGameModeFlashTimer > 0 && ((this.altGameModeFlashTimer -= e), this.tRex.update(e), (e = 0)),
				(this.time = t),
				this.playing)
			) {
				this.clearCanvas(),
					this.altGameModeActive && this.fadeInTimer <= this.config.FADE_DURATION
						? ((this.fadeInTimer += e / 1e3), (this.canvasCtx.globalAlpha = this.fadeInTimer))
						: (this.canvasCtx.globalAlpha = 1),
					this.tRex.jumping && this.tRex.updateJump(e),
					(this.runningTime += e);
				const t = this.runningTime > this.config.CLEAR_TIME;
				if ((1 !== this.tRex.jumpCount || this.playingIntro || this.playIntro(), this.playingIntro))
					this.horizon.update(0, this.currentSpeed, t);
				else if (!this.crashed) {
					const i = this.isDarkMode ^ this.inverted;
					(e = this.activated ? e : 0), this.horizon.update(e, this.currentSpeed, t, i);
				}
				...
				if ((!Runner.audioCues && s && this.playSound(this.soundFx.SCORE), !Runner.isAltGameModeEnabled()))
					if (this.invertTimer > this.config.INVERT_FADE_DURATION) (this.invertTimer = 0), (this.invertTrigger = !1), this.invert(!1);
						...
						t > 0 &&
							...
							this.invertTrigger && 0 === this.invertTimer && ((this.invertTimer += e), this.invert(!1)));
					}
			}
			(this.playing || (!this.activated && this.tRex.blinkCount < Runner.config.MAX_BLINK_COUNT)) &&
		...
		onKeyDown(t) {
			if ((IS_MOBILE && this.playing && t.preventDefault(), this.isCanvasInView())) {
				if (Runner.keycodes.JUMP[t.keyCode] && t.target == this.slowSpeedCheckbox) return;
				if (!this.crashed && !this.paused) {
					const e =
						(IS_MOBILE && t.type === Runner.events.POINTERDOWN && 'mouse' == t.pointerType && t.target == this.containerEl) ||
						(IS_IOS && 'touch' == t.pointerType && document.activeElement == this.containerEl);
					Runner.keycodes.JUMP[t.keyCode] ||
		...
		onKeyUp(t) {
			const e = String(t.keyCode),
				i = Runner.keycodes.JUMP[e] || t.type === Runner.events.TOUCHEND || t.type === Runner.events.POINTERUP;
			if (this.isRunning() && i) this.tRex.endJump();
			else if (Runner.keycodes.DUCK[e]) (this.tRex.speedDrop = !1), this.tRex.setDuck(!1);
			else if (this.crashed) {
				const i = getTimeStamp() - this.time;
				this.isCanvasInView() &&
					(Runner.keycodes.RESTART[e] ||
						this.isLeftClickOnCanvas(t) ||
						(i >= this.config.GAMEOVER_CLEAR_TIME && Runner.keycodes.JUMP[e])) &&
		...
		pollGamepadState() {
			...
			this.pollActiveGamepad(t), (this.pollingGamepads = !0), requestAnimationFrame(this.pollGamepadState.bind(this));
		},
		pollForActiveGamepad(t) {
			for (let e = 0; e < t.length; ++e)
				if (t[e] && t[e].buttons.length > 0 && t[e].buttons[0].pressed) return (this.gamepadIndex = e), void this.pollActiveGamepad(t);
		},
		pollActiveGamepad(t) {
			if (void 0 === this.gamepadIndex) return void this.pollForActiveGamepad(t);
			const e = t[this.gamepadIndex];
			if (!e) return (this.gamepadIndex = void 0), void this.pollForActiveGamepad(t);
		...
		pollGamepadButton(t, e, i) {
			...
			let n = !1;
			if ((this.previousGamepad && (n = this.previousGamepad.buttons[e].pressed), s !== n)) {
		...
		isLeftClickOnCanvas(t) {
			return (
				...
				(t.target === this.canvas || (IS_MOBILE && Runner.audioCues && t.target === this.containerEl))
			);
		},
		scheduleNextUpdate() {
		...
		initializeHighScore(t) {
			(this.syncHighestScore = !0),
				(t = Math.ceil(t)) < this.highestScore
					...
					: ((this.highestScore = t), this.distanceMeter.setHighScore(this.highestScore));
		},
		saveHighScore(t, e) {
		...
		gameOver() {
			if (
				...
				this.canvas &&
					(Runner.isAltGameModeEnabled
						...
						: (this.gameOverPanel = new GameOverPanel(this.canvas, t.TEXT_SPRITE, t.RESTART, this.dimensions)));
			}
			this.gameOverPanel.draw(this.altGameModeActive, this.tRex),
				...
				Runner.audioCues &&
					...
					announcePhrase(
						getA11yString(A11Y_STRINGS.gameOver).replace('$1', this.distanceMeter.getActualDistance(this.distanceRan).toString()) +
							' ' +
							getA11yString(A11Y_STRINGS.highScore).replace('$1', this.distanceMeter.getActualDistance(this.highestScore).toString()),
					),
		...
		stop() {
			this.setPlayStatus(!1), (this.paused = !0), cancelAnimationFrame(this.raqId), (this.raqId = 0), this.generatedSoundFx.stopAll();
		},
		play() {
			this.crashed ||
				(this.setPlayStatus(!0),
		...
		setPlayStatus(t) {
			this.touchController && this.touchController.classList.toggle(HIDDEN_CLASS, !t), (this.playing = t);
		},
		isArcadeMode: () => (IS_RTL ? 1 == document.title.indexOf(ARCADE_MODE_URL) : document.title === ARCADE_MODE_URL),
		setArcadeMode() {
			document.body.classList.add(Runner.classes.ARCADE_MODE), this.setArcadeModeContainerScale();
		},
		setArcadeModeContainerScale() {
			const t = window.innerHeight,
				...
				a =
					Math.ceil(
						Math.max(0, (t - n - Runner.config.ARCADE_MODE_INITIAL_TOP_POSITION) * Runner.config.ARCADE_MODE_TOP_POSITION_PERCENT),
					) * window.devicePixelRatio,
				o = IS_RTL ? -s + ',' + s : s;
			this.containerEl.style.transform = 'scale(' + o + ') translateY(' + a + 'px)';
		},
		onVisibilityChange(t) {
			document.hidden || document.webkitHidden || 'blur' === t.type || 'visible' !== document.visibilityState
				? this.stop()
		...
		invert(t) {
			...
			t
				? (e.classList.toggle(Runner.classes.INVERTED, !1), (this.invertTimer = 0), (this.inverted = !1))
	...
	(GeneratedSoundFx.prototype = {
		...
		background() {
			if (this.audioCues) {
				...
				this.playNote(493.883, t, 0.116), this.playNote(659.255, t + 0.116, 0.232), this.loopFootSteps();
			}
		},
		loopFootSteps() {
			this.audioCues &&
				...
				(this.bgSoundIntervalId = setInterval(
					function () {
						this.playNote(73.42, this.context.currentTime, 0.05, 0.16), this.playNote(69.3, this.context.currentTime + 0.116, 0.116, 0.16);
					}.bind(this),
					280,
				));
		},
		...
		collect() {
			if (this.audioCues) {
				this.cancelFootSteps();
				const t = this.context.currentTime;
				this.playNote(830.61, t, 0.116), this.playNote(1318.51, t + 0.116, 0.232);
			}
		},
		jump() {
			if (this.audioCues) {
				const t = this.context.currentTime;
				this.playNote(659.25, t, 0.116, 0.3, -0.6), this.playNote(880, t + 0.116, 0.232, 0.3, -0.6);
			}
		},
	}),
	...
	(GameOverPanel.prototype = {
		...
		drawGameOverText(t, e) {
			...
			const h = Math.round(i - t.TEXT_WIDTH / 2),
				...
				c = t.TEXT_WIDTH,
				l = t.TEXT_HEIGHT;
			IS_HIDPI && ((n *= 2), (s *= 2), (a *= 2), (o *= 2)), e || ((s += this.textImgPos.x), (n += this.textImgPos.y));
		...
		update() {
			...
			if (
				...
				(this.animTimer += e),
				(this.flashTimer += e),
				0 == this.currentFrame && this.animTimer > GameOverPanel.LOGO_PAUSE_DURATION)
			)
				...
				this.animTimer >= GameOverPanel.animConfig.msPerFrame && (this.currentFrame++, this.drawRestartButton());
			else if (!this.altGameModeActive && this.currentFrame == GameOverPanel.animConfig.frames.length) return void this.reset();
			if (this.altGameModeActive && Runner.spriteDefinitionByType.original.ALT_GAME_OVER_TEXT_CONFIG) {
				...
				else if (this.flashCounter >= GameOverPanel.FLASH_ITERATIONS) return void this.reset();
			}
	...
	(Obstacle.prototype = {
			...
			this.remove ||
				...
				this.typeConfig.numFrames &&
					...
					this.timer >= this.typeConfig.frameRate &&
						((this.currentFrame = this.currentFrame === this.typeConfig.numFrames - 1 ? 0 : this.currentFrame + 1), (this.timer = 0))),
				this.draw(),
		...
		cloneCollisionBoxes() {
			...
			for (let e = t.length - 1; e >= 0; e--) this.collisionBoxes[e] = new CollisionBox(t[e].x, t[e].y, t[e].width, t[e].height);
		},
	}),
	(Trex.config = {
		DROP_VELOCITY: -5,
	...
	(Trex.prototype = {
		...
		enableSlowConfig: function () {
			...
			(Trex.config = Object.assign(Trex.config, t)), this.adjustAltGameConfigForSlowSpeed();
		},
		...
		setJumpVelocity(t) {
			(this.config.INITIAL_JUMP_VELOCITY = -t), (this.config.DROP_VELOCITY = -t / 2);
		},
		update(t, e) {
			(this.timer += t),
				e &&
					...
					e === Trex.status.WAITING && ((this.animStartTime = getTimeStamp()), this.setBlinkDelay())),
				this.playingIntro &&
					this.xPos < this.config.START_X_POS &&
					((this.xPos += Math.round((this.config.START_X_POS / this.config.INTRO_DURATION) * t)), (this.xInitialPos = this.xPos)),
				...
				!this.flashing &&
					...
					((this.currentFrame = this.currentFrame == this.currentAnimFrames.length - 1 ? 0 : this.currentFrame + 1), (this.timer = 0)),
				this.altGameModeEnabled || (this.speedDrop && this.yPos === this.groundYPos && ((this.speedDrop = !1), this.setDuck(!0)));
		},
		draw(t, e) {
			let i = t,
				s = e,
			...
			this.altGameModeEnabled && this.jumping && this.status !== Trex.status.CRASHED && (n = this.config.WIDTH_JUMP),
				...
				(s += this.spritePos.y),
				this.flashing &&
					(this.timer < this.config.FLASH_ON ? (this.canvasCtx.globalAlpha = 0.5) : this.timer > this.config.FLASH_OFF && (this.timer = 0)),
				!this.altGameModeEnabled && this.ducking && this.status !== Trex.status.CRASHED
					? this.canvasCtx.drawImage(Runner.imageSprite, i, s, n, a, this.xPos, this.yPos, this.config.WIDTH_DUCK, o)
					: this.altGameModeEnabled && this.jumping && this.status !== Trex.status.CRASHED
		...
		blink(t) {
			t - this.animStartTime >= this.blinkDelay &&
				...
				1 === this.currentFrame && (this.setBlinkDelay(), (this.animStartTime = t), this.blinkCount++));
		},
		startJump(t) {
			this.jumping ||
		...
		updateJump(t) {
			...
			this.speedDrop
				? (this.yPos += Math.round(this.jumpVelocity * this.config.SPEED_DROP_COEFFICIENT * e))
				...
				((this.config.INVERT_JUMP && this.yPos > -this.config.MAX_JUMP_HEIGHT) ||
					...
					this.speedDrop) &&
					this.endJump(),
				((this.config.INVERT_JUMP && this.yPos) < this.groundYPos || (!this.config.INVERT_JUMP && this.yPos) > this.groundYPos) &&
					(this.reset(), this.jumpCount++, Runner.audioCues && Runner.generatedSoundFx.loopFootSteps());
		},
		setSpeedDrop() {
		...
		setDuck(t) {
			t && this.status !== Trex.status.DUCKING
				...
				: this.status === Trex.status.DUCKING && (this.update(0, Trex.status.RUNNING), (this.ducking = !1));
		},
		reset() {
	...
	(DistanceMeter.prototype = {
		...
		draw(t, e, i) {
			...
			if ((IS_HIDPI && ((s *= 2), (n *= 2), (a *= 2)), (a += this.spritePos.x), (o += this.spritePos.y), this.canvasCtx.save(), IS_RTL))
				i
			...
			this.canvasCtx.drawImage(this.image, a, o, s, n, h, r, c, l), this.canvasCtx.restore();
		},
		getActualDistance(t) {
		...
		update(t, e) {
				...
				this.flashIterations <= this.config.FLASH_ITERATIONS
					? ((this.flashTimer += t),
						this.flashTimer < this.config.FLASH_DURATION
							? (i = !1)
							: this.flashTimer > 2 * this.config.FLASH_DURATION && ((this.flashTimer = 0), this.flashIterations++))
					: ((this.achievement = !1), (this.flashIterations = 0), (this.flashTimer = 0));
			else if (
				((e = this.getActualDistance(e)) > this.maxScore && this.maxScoreUnits == this.config.MAX_DISTANCE_UNITS
					? (this.maxScoreUnits++, (this.maxScore = parseInt(this.maxScore + '9', 10)))
					: (this.distance = 0),
				e > 0)
			) {
			...
			if (i) for (let t = this.digits.length - 1; t >= 0; t--) this.draw(t, parseInt(this.digits[t], 10));
			return this.drawHighScore(), s;
		},
		drawHighScore() {
			if (parseInt(this.highScore, 10) > 0) {
				this.canvasCtx.save(), (this.canvasCtx.globalAlpha = 0.8);
				for (let t = this.highScore.length - 1; t >= 0; t--) this.draw(t, parseInt(this.highScore[t], 10), !0);
				this.canvasCtx.restore();
			}
		},
		setHighScore(t) {
		...
		update(t) {
			this.remove ||
				(this.spriteConfig.FIXED
					? ((this.animTimer += t),
						this.animTimer > BackgroundEl.config.MS_PER_FRAME && ((this.animTimer = 0), (this.switchFrames = !this.switchFrames)),
	...
	(NightMode.prototype = {
		update(t) {
			if (
				(t && 0 === this.opacity && (this.currentPhase++, this.currentPhase >= NightMode.phases.length && (this.currentPhase = 0)),
				t && (this.opacity < 1 || 0 === this.opacity)
					? (this.opacity += NightMode.config.FADE_SPEED)
					: this.opacity > 0 && (this.opacity -= NightMode.config.FADE_SPEED),
				this.opacity > 0)
			) {
				if (((this.xPos = this.updateXPos(this.xPos, NightMode.config.MOON_SPEED)), this.drawStars))
			...
			for (let e = 0; e < NightMode.config.NUM_STARS; e++)
				(this.stars[e] = {}),
					...
					(this.stars[e].sourceY = IS_HIDPI
						? Runner.spriteDefinitionByType.original.HDPI.STAR.y + 2 * NightMode.config.STAR_SIZE * e
						: Runner.spriteDefinitionByType.original.LDPI.STAR.y + NightMode.config.STAR_SIZE * e);
		},
		reset() {
	...
	(HorizonLine.prototype = {
		...
		update(t, e) {
			...
			this.xPos[0] <= 0 ? this.updateXPos(0, i) : this.updateXPos(1, i), this.draw();
		},
		reset() {
	...
	(Horizon.prototype = {
		init() {
			(Obstacle.types = Runner.spriteDefinitionByType.original.OBSTACLES), this.addCloud();
			...
			for (let t = 0; t < Obstacle.types.length; t++)
				Runner.slowDown &&
					...
					'object' == typeof Obstacle.types[t].yPos &&
						((Obstacle.types[t].yPos = Obstacle.types[t].yPos[0]), (Obstacle.types[t].yPosMobile = Obstacle.types[t].yPos[0])));
		},
		enableAltGameMode: function (t) {
			(this.clouds = []),
				(this.backgroundEls = []),
		...
		update(t, e, i, s) {
			(this.runningTime += t), this.altGameModeActive && this.updateBackgroundEls(t, e);
			for (let i = 0; i < this.horizonLines.length; i++) this.horizonLines[i].update(t, e);
		...
		addNewObstacle(t) {
			const e =
				...
				s = Obstacle.types[i];
			if ((e > 0 && this.duplicateObstacleCheck(s.type)) || t < s.minSpeed) this.addNewObstacle(t);
			else {
		...
		duplicateObstacleCheck(t) {
			let e = 0;
			for (let i = 0; i < this.obstacleHistory.length; i++) e = this.obstacleHistory[i] === t ? e + 1 : 0;
			return e >= Runner.config.MAX_OBSTACLE_DUPLICATION;
		},
		reset() {
			this.obstacles = [];
			for (let t = 0; t < this.horizonLines.length; t++) this.horizonLines[t].reset();
		...
		addBackgroundEl() {
			...
			if (t.length > 0) {
				...
				(this.lastEl = i), this.backgroundEls.push(new BackgroundEl(this.canvas, this.spritePos.BACKGROUND_EL, this.dimensions.WIDTH, i));
			}
		},
	});
